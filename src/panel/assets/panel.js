...
function renderRow(e) {
  ...
  const preview = `
    <tr class="row ${open ? "is-open" : ""}" data-id="${escapeHtml(e.id)}">
      ...
      <td class="mono muted">${escapeHtml(e.provider || "")}</td>
...
function ioCell(value, extra) {
  if (!value) return `<div class="io ${extra} empty"></div>`;
  ...
  return `
    <tr class="detail">
      <td colspan="${PREVIEW_COLUMNS}">
        ...
        <div class="meta-line">
          <span>session <b>${escapeHtml(e.sessionId)}</b></span>
          <span>service <b>${escapeHtml(e.service)}</b></span>
          <span>listen <b>${escapeHtml(e.address)}</b></span>
          ${e.provider ? `<span>provider <b>${escapeHtml(e.provider)}</b></span>` : ""}
          ${e.latencyMs != null ? `<span>latency <b>${e.latencyMs} ms</b></span>` : ""}
          <span>${escapeHtml(new Date(e.ts).toISOString())}</span>
        </div>
      </td>
    </tr>`;
}
