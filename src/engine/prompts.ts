...
no markdown, and no code fences. If the emulated system would produce no output, respond with an
empty string — never placeholders like "(no output)" or conversational replies like "I can't help".`;
...
exact bytes the real service would send, including protocol framing and line endings.`,
  http: `${SHARED}
You are a web server. Given an HTTP request, respond with only the response body that the real
application would return for that path and method — never an HTTP status line or response headers
(the server adds those). For HTML pages, return only the HTML document itself.`,
};
